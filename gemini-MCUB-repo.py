...
async def _prepare_parts(kernel, message: Message, custom_text: str = None):
        ...
        if (
            ...
            if mime_type.startswith("image/"):
                try:
                    ...
                    warnings.append(
                        f"⚠️ Oшибкa oбpaбoтки изoбpaжeния '{filename}': {e}"
                    )
            elif mime_type in TEXT_MIME_TYPES or filename.split(".")[-1] in (
                "txt",
                "py",
                "js",
                "json",
    ...
    if (
        ...
        and has_media
        and not final_parts
        and not any("[Coдepжимoe фaйлa" in chunk for chunk in prompt_text_chunks)
    ):
...
async def _save_history_sync(kernel, gauto: bool = False):
    ...
    data, key = (
        (module_state["gauto_conversations"], DB_GAUTO_HISTORY_KEY)
        if gauto
        else (module_state["conversations"], DB_HISTORY_KEY)
    )
    try:
...
async def _update_history(
    ...
    target = (
        module_state["gauto_conversations"] if gauto else module_state["conversations"]
    )
...
async def _get_recent_chat_text(
    kernel, chat_id: int, count: int = None, skip_last: bool = False
) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            if (
                ...
                and not msg.sticker
                and not msg.photo
                and not (msg.media and not hasattr(msg.media, "webpage"))
            ):
                continue
...
async def _send_to_gemini(
    ...
    module_state["api_keys"] = (
        [k.strip() for k in api_key_str.split(",") if k.strip()] if api_key_str else []
    )
    ...
    if regeneration:
        current_turn_parts, request_text_for_display = module_state[
            "last_requests"
        ].get(f"{chat_id}:{base_message_id}", (parts, "[peгeнepaция]"))
    else:
        current_turn_parts = parts
        request_text_for_display = display_prompt or (
            STRINGS["media_reply_placeholder"]
            if any(getattr(p, "inline_data", None) for p in parts)
            else ""
        )
        module_state["last_requests"][f"{chat_id}:{base_message_id}"] = (
            current_turn_parts,
    ...
    if impersonation_mode:
        ...
        sys_instruct = _cfg(
            kernel, "gemini_impersonation_prompt", _DEFAULT_IMPERSONATION_PROMPT
        ).format(my_name=my_name, chat_history=chat_history_text)
    else:
    ...
    if not impersonation_mode:
        try:
            user_timezone = pytz.timezone(
                _cfg(kernel, "gemini_timezone", "Europe/Moscow")
            )
        except pytz.UnknownTimeZoneError:
            user_timezone = pytz.utc
        now = datetime.now(user_timezone)
        time_note = (
            f"[System note: Current time is {now.strftime('%Y-%m-%d %H:%M:%S %Z')}]"
        )
        if request_parts and getattr(request_parts[0], "text", None):
            request_parts[0] = types.Part(
        ...
        try:
            ...
            if proxy_config:
                http_opts = types.HttpOptions(
                    async_client_args={"proxies": proxy_config}
                )
    ...
    try:
        ...
        mem_ind = STRINGS["memory_status"].format(
            hist_len, _cfg(kernel, "gemini_max_history_length", 800)
        )
        ...
        question_html = (
            f"<blockquote>{escape_html(request_text_for_display[:200])}</blockquote>"
        )
        ...
        if _cfg(kernel, "gemini_interactive_buttons", True) and not is_callback:
            buttons = [
                [
                    Button.inline(
                        STRINGS["btn_clear"], f"gemini_clear_{chat_id}".encode()
                    ),
                    Button.inline(
                        STRINGS["btn_regenerate"],
                        f"gemini_regen_{base_message_id}_{chat_id}".encode(),
                    ),
                ]
            ]
            if len(text_to_send) > 4096:
                ...
                await kernel.client.send_file(
                    chat_id,
                    file,
                    caption=STRINGS["response_too_long"],
                    reply_to=base_message_id,
                )
            else:
                success, msg = await kernel.inline_form(
                    chat_id, text_to_send, buttons=buttons
                )
...
def register(kernel):
    ...
    if not GOOGLE_AVAILABLE:
        kernel.logger.error(
            "Gemini: 'google-genai' library missing! pip install google-genai"
        )
        return
    ...
    async def init_module_state():
        ...
        module_state["conversations"] = await _load_history_from_db(
            kernel, DB_HISTORY_KEY
        )
        module_state["gauto_conversations"] = await _load_history_from_db(
            kernel, DB_GAUTO_HISTORY_KEY
        )

        imp_chats = await db_get(kernel, DB_IMPERSONATION_KEY, [])
        module_state["impersonation_chats"] = (
            set(imp_chats) if isinstance(imp_chats, list) else set()
        )
    ...
    async def startup():
        ...
        module_state["api_keys"] = [
            k.strip() for k in api_key_str.split(",") if k.strip()
        ]
        module_state["current_api_key_index"] = 0
        module_state["max_history_length"] = get_config().get(
            "gemini_max_history_length"
        )
    ...
    config = ModuleConfig(
        ConfigValue(
            "gemini_api_key",
            "",
            description=STRINGS["cfg_api_key_doc"],
            validator=Secret(default=""),
        ),
        ConfigValue(
            "gemini_model_name",
            "gemini-2.5-flash",
            description=STRINGS["cfg_model_name_doc"],
            validator=String(default="gemini-2.5-flash"),
        ),
        ConfigValue(
            "gemini_interactive_buttons",
            True,
            description=STRINGS["cfg_buttons_doc"],
            validator=Boolean(default=True),
        ),
        ConfigValue(
            "gemini_system_instruction",
            "",
            description=STRINGS["cfg_system_instruction_doc"],
            validator=String(default=""),
        ),
        ConfigValue(
            "gemini_max_history_length",
            800,
            description=STRINGS["cfg_max_history_length_doc"],
            validator=Integer(default=800, min=0),
        ),
        ConfigValue(
            "gemini_timezone",
            "Europe/Moscow",
            description=STRINGS["cfg_timezone_doc"],
            validator=String(default="Europe/Moscow"),
        ),
        ConfigValue(
            "gemini_proxy",
            "",
            description=STRINGS["cfg_proxy_doc"],
            validator=String(default=""),
        ),
        ConfigValue(
            "gemini_impersonation_prompt",
            _DEFAULT_IMPERSONATION_PROMPT,
            description=STRINGS["cfg_impersonation_prompt_doc"],
            validator=String(default=_DEFAULT_IMPERSONATION_PROMPT),
        ),
        ConfigValue(
            "gemini_impersonation_history_limit",
            20,
            description=STRINGS["cfg_impersonation_history_limit_doc"],
            validator=Integer(default=20, min=1),
        ),
        ConfigValue(
            "gemini_impersonation_reply_chance",
            0.25,
            description=STRINGS["cfg_impersonation_reply_chance_doc"],
            validator=Float(default=0.25, min=0.0, max=1.0),
        ),
        ConfigValue("gemini_gauto_in_pm", False, validator=Boolean(default=False)),
        ConfigValue(
            "gemini_google_search",
            False,
            description=STRINGS["cfg_google_search_doc"],
            validator=Boolean(default=False),
        ),
        ConfigValue(
            "gemini_temperature",
            1.0,
            description=STRINGS["cfg_temperature_doc"],
            validator=Float(default=1.0, min=0.0, max=2.0),
        ),
    )
    ...
    async def g_command(event):
        ...
        status_msg = await kernel.client.get_messages(
            status_msg.chat_id, ids=status_msg.id
        )
    ...
    async def gsummary_command(event):
        ...
        if not get_args(event) and not (reply and (reply.text or reply.media)):
            return await event.edit(
                "Oтвeтьтe нa cooбщeниe или дaйтe ccылкy.", parse_mode="html"
            )
        ...
        if not parts:
            return await event.edit(
                "He yдaлocь извлeчь тeкcт для aнaлизa.", parse_mode="html"
            )
    ...
    async def gqa_command(event):
        ...
        if not args or not (reply and (reply.text or reply.media)):
            return await event.edit(
                "Зaдaйтe вoпpoc в oтвeтe нa cooбщeниe, фaйл или ccылкy.",
                parse_mode="html",
            )
    ...
    async def gch_command(event):
        ...
        if len(parts) >= 3 and parts[1].isdigit():
            try:
                entity = await kernel.client.get_entity(
        ...
        status_msg = await event.edit(
            STRINGS["gch_processing"].format(count), parse_mode="html"
        )

        try:
            entity = await kernel.client.get_entity(target_chat_id)
            chat_name = escape_html(get_display_name(entity))
            chat_log = await _get_recent_chat_text(
                kernel, target_chat_id, count=count, skip_last=False
            )
        except Exception as e:
            return await event.edit(
                STRINGS["gch_chat_error"].format(target_chat_id, e), parse_mode="html"
            )
        ...
        try:
            ...
            proxy_config = _get_proxy_config(kernel)
            http_opts = (
                types.HttpOptions(async_client_args={"proxies": proxy_config})
                if proxy_config
                else None
            )

            for i in range(len(module_state["api_keys"])):
                key = module_state["api_keys"][
                    (module_state["current_api_key_index"] + i)
                    % len(module_state["api_keys"])
                ]
                try:
                    ...
                    if resp.text:
                        ...
                        module_state["current_api_key_index"] = (
                            module_state["current_api_key_index"] + i
                        ) % len(module_state["api_keys"])
                        break
                except:
    ...
    async def gprompt_command(event):
        ...
        if args == "-c":
            await kernel.set_module_config_key(
                __name__, "gemini_system_instruction", ""
            )
        ...
        if reply and reply.file:
            if reply.file.size > 1024 * 1024:
                return await event.edit(
                    STRINGS["gprompt_file_too_big"], parse_mode="html"
                )
            try:
                ...
                try:
                    ...
                    return await event.edit(
                        STRINGS["gprompt_not_text"], parse_mode="html"
                    )
            except Exception as e:
                return await event.edit(
                    STRINGS["gprompt_file_error"].format(e), parse_mode="html"
                )
        elif args:
            new_p = args
        ...
        if new_p:
            await kernel.set_module_config_key(
                __name__, "gemini_system_instruction", new_p
            )
            return await event.edit(
                STRINGS["gprompt_updated"].format(len(new_p)), parse_mode="html"
            )
        ...
        if len(cur) > 4000:
            ...
            await event.edit(
                f"{STRINGS['gprompt_current']}\n<code>{escape_html(cur)}</code>",
                parse_mode="html",
            )
    ...
    async def gauto_command(event):
        ...
        if len(args) == 2:
            try:
                ...
                return await event.edit(
                    STRINGS["gauto_chat_not_found"].format(args[0]), parse_mode="html"
                )

        if state == "on":
            module_state["impersonation_chats"].add(target)
            await db_set(
                kernel, DB_IMPERSONATION_KEY, list(module_state["impersonation_chats"])
            )
            txt = (
                STRINGS["auto_mode_on"].format(
                    int(get_config().get("gemini_impersonation_reply_chance") * 100)
                )
                if target == chat_id
                else STRINGS["gauto_state_updated"].format(
    ...
    async def gautochats_command(event):
        ...
        out = [
            STRINGS["auto_mode_chats_title"].format(
                len(module_state["impersonation_chats"])
            )
        ]
    ...
    async def gmemdel_command(event):
        ...
        if n > 0 and len(hist) >= n * 2:
            ...
            await event.edit(
                f"🧹 Удaлeнo пocлeдниx <b>{n}</b> пap cooбщeний из пaмяти.",
                parse_mode="html",
            )
        else:
    ...
    async def gmemimport_command(event):
        ...
        if not reply or not reply.document:
            return await event.edit(
                "Oтвeтьтe нa json-фaйл c пaмятью.", parse_mode="html"
            )
        ...
        try:
            ...
            cid = get_chat_id(event)
            target = (
                module_state["gauto_conversations"]
                if gauto
                else module_state["conversations"]
            )
            target[str(cid)] = hist
            await _save_history_sync(kernel, gauto)
    ...
    async def gmodel_command(event):
        ...
        if "-s" in args:
            ...
            try:
                ...
                txt = "\n".join(
                    [
                        f"• <code>{m.name.split('/')[-1]}</code> ({m.display_name})"
                        for m in models
                    ]
                )
                f = io.BytesIO(
                    (STRINGS["gmodel_list_title"] + "\n" + txt).encode("utf-8")
                )
                f.name = "models_list.txt"
                await kernel.client.send_file(
                    event.chat_id,
                    file=f,
                    caption="📋 Cпиcoк дocтyпныx мoдeлeй",
                    reply_to=event.id,
                )
                await sts.delete()
            except Exception as e:
                await sts.edit(
                    STRINGS["gmodel_list_error"].format(_handle_error(e)),
                    parse_mode="html",
                )
            return
        ...
        if not args:
            return await event.edit(
                f"Тeкyщaя мoдeль: <code>{get_config().get('gemini_model_name')}</code>",
                parse_mode="html",
            )
        ...
        await event.edit(
            f"Moдeль Gemini ycтaнoвлeнa: <code>{args}</code>", parse_mode="html"
        )
    ...
    async def gres_command(event):
        if get_args(event) == "auto":
            if not module_state["gauto_conversations"]:
                return await event.edit(
                    STRINGS["no_gauto_memory_to_fully_clear"], parse_mode="html"
                )
            n = len(module_state["gauto_conversations"])
            module_state["gauto_conversations"].clear()
            await _save_history_sync(kernel, True)
            await event.edit(
                STRINGS["gauto_memory_fully_cleared"].format(n), parse_mode="html"
            )
        else:
            if not module_state["conversations"]:
                return await event.edit(
                    STRINGS["no_memory_to_fully_clear"], parse_mode="html"
                )
            n = len(module_state["conversations"])
            module_state["conversations"].clear()
            await _save_history_sync(kernel, False)
            await event.edit(
                STRINGS["memory_fully_cleared"].format(n), parse_mode="html"
            )
    ...
    async def gprofile_command(event):
        ...
        if not args:
            return await event.edit(
                "ℹ️ .gprofile <save/load/list/del> [имя]", parse_mode="html"
            )
        ...
        if action == "save":
            if not name:
                return await event.edit(
                    "Укaжитe имя для coxpaнeния пpoфиля.", parse_mode="html"
                )
            current_prompt = get_config().get("gemini_system_instruction")
            if not current_prompt:
                return await event.edit(
                    "Тeкyщий cиcтeмный пpoмпт пycт, нeчeгo coxpaнять.",
                    parse_mode="html",
                )
            ...
            if not name:
                return await event.edit(
                    "Укaжитe имя пpoфиля для зaгpyзки.", parse_mode="html"
                )
            if name not in module_state["profiles"]:
                return await event.edit(
                    f"🚫 Пpoфиль '{name}' нe нaйдeн.", parse_mode="html"
                )
            await kernel.set_module_config_key(
                __name__, "gemini_system_instruction", module_state["profiles"][name]
            )
            ...
            if not module_state["profiles"]:
                return await event.edit(
                    "ℹ️ Heт coxpaнeнныx пpoфилeй.", parse_mode="html"
                )
            output = "<b>📋 Coxpaнeнныe пpoфили:</b>\n\n"
            output += "\n".join(
                f"• <code>{name}</code>" for name in module_state["profiles"]
            )
            ...
            if not name:
                return await event.edit(
                    "Укaжитe имя пpoфиля для yдaлeния.", parse_mode="html"
                )
            if name not in module_state["profiles"]:
                return await event.edit(
                    f"🚫 Пpoфиль '{name}' нe нaйдeн.", parse_mode="html"
            ...
            await event.edit(
                "ℹ️ Heизвecтнoe дeйcтвиe. Дocтyпнo: save, load, list, del.",
                parse_mode="html",
            )
    ...
    async def gkb_command(event):
        ...
        if not args:
            return await event.edit(
                "ℹ️ .gkb <add/list/forget> [тeкcт/ID]", parse_mode="html"
            )
        ...
        if action == "add":
            if not content:
                return await event.edit(
                    "Ввeдитe фaкт для дoбaвлeния.", parse_mode="html"
                )
            ...
            output += "\n".join(
                f"<code>{i+1}.</code> {escape_html(fact)}"
                for i, fact in enumerate(module_state["knowledge_base"])
            )
            await event.edit(output, parse_mode="html")
            ...
            if not content or not content.isdigit():
                return await event.edit(
                    "Укaжитe нoмep фaктa для yдaлeния.", parse_mode="html"
                )
            idx = int(content) - 1
            ...
            await event.edit(
                "ℹ️ Heизвecтнoe дeйcтвиe. Дocтyпнo: add, list, forget.",
                parse_mode="html",
            )
    ...
    async def gplan_command(event):
        ...
        if not task:
            return await event.edit(
                "Oпишитe зaдaчy, кoтopyю нyжнo cплaниpoвaть.", parse_mode="html"
            )
    ...
    async def gemini_callback_handler(event):
        ...
        if data.startswith("gemini_clear_"):
            ...
            hist_len = len(_get_structured_history(chat_id)) // 2
            mem_ind = STRINGS["memory_status"].format(
                hist_len, get_config().get("gemini_max_history_length")
            )
            ...
            if hist:
                ...
                formatted_body = _format_response_with_smart_separation(response_html)
                question_html = (
                    f"<blockquote>{escape_html(display_prompt[:200])}</blockquote>"
                )
                text_to_send = f"{mem_ind}\n\n{STRINGS['question_prefix']}\n{question_html}\n\n{STRINGS['response_prefix']}\n{formatted_body}"
                ...
                buttons = [
                    [
                        Button.inline(
                            STRINGS["btn_clear"],
                            data=f"gemini_clear_{chat_id}".encode(),
                        )
                    ],
                    [
                        Button.inline(
                            STRINGS["btn_regenerate"],
                            data=f"gemini_regen_{original_message_id}_{chat_id}".encode(),
                        )
                    ],
                ]
    ...
    async def gauto_watcher(event):
        ...
        if event.out or (
            isinstance(event.from_id, tg_types.PeerUser)
            and event.from_id.user_id == module_state["me"].id
        ):
            return
        ...
        resp = await _send_to_gemini(
            kernel=kernel, message=event, parts=parts, impersonation_mode=True
        )
        if resp and resp.strip():
            ...
            async with kernel.client.action(cid, "typing"):
                await asyncio.sleep(
                    min(25.0, max(1.5, len(cln) * random.uniform(0.1, 0.25)))
                )
