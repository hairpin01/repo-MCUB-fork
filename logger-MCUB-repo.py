...
def register(kernel):
    ...
    async def init_db():
        ...
        try:
            async with aiosqlite.connect(DB_FILE) as db:
                ...
                await db.commit()
                kernel.cprint(
                    f"{kernel.Colors.GREEN}✅ Logger database initialized{kernel.Colors.RESET}"
                )
        ...
        try:
            async with aiosqlite.connect(DB_FILE) as db:
                cursor = await db.execute(
                    "SELECT chat_id FROM logging WHERE chat_id = ?", (chat_id,)
                )
    ...
    async def log_cmd(event):
        try:
            ...
            if len(args) < 2:
                await event.edit(
                    "⚪ <b>usage:</b> .log on/off/clear/deleted/edited",
                    parse_mode="html",
                )
                return
            ...
            if subcmd == "on":
                ...
                async with aiosqlite.connect(DB_FILE) as db:
                    await db.execute(
                        "INSERT OR IGNORE INTO logging VALUES (?)", (event.chat_id,)
                    )
                    await db.commit()
                await event.edit(
                    "✅ <b>logging enabled for this chat</b>", parse_mode="html"
                )
                await event.delete()
                ...
                async with aiosqlite.connect(DB_FILE) as db:
                    await db.execute(
                        "DELETE FROM logging WHERE chat_id = ?", (event.chat_id,)
                    )
                    await db.commit()
                await event.edit(
                    "🔕 <b>logging disabled for this chat</b>", parse_mode="html"
                )
                await event.delete()
                ...
                async with aiosqlite.connect(DB_FILE) as db:
                    await db.execute(
                        "DELETE FROM deleted WHERE chat_id = ?", (event.chat_id,)
                    )
                    await db.execute(
                        "DELETE FROM edited WHERE chat_id = ?", (event.chat_id,)
                    )
                    await db.commit()
    ...
    async def show_deleted(event):
        try:
            ...
            async with aiosqlite.connect(DB_FILE) as db:
                cursor = await db.execute(
                    """SELECT * FROM deleted WHERE chat_id = ?
    ...
    async def show_edited(event):
        try:
            ...
            async with aiosqlite.connect(DB_FILE) as db:
                cursor = await db.execute(
                    """SELECT * FROM edited WHERE chat_id = ?
                                         ORDER BY timestamp DESC LIMIT 100""",
                    (event.chat_id,),
                )
                msgs = await cursor.fetchall()
